...
class Core {
    ...
    public function get_data($connection) {
        ...
        if ($google_business != null) {
            ...
            foreach ($google_business as $biz) {
                ...
                if (substr($biz->id, 0, 9) === "accounts/") {
                    $result = $this->get_google_reviews_all($biz, $options);
                } else {
        ...
        if ($facebook_business != null) {
            foreach ($facebook_business as $biz) {
                ...
                $result = $this->get_facebook_reviews($biz, $access_token, $options);
        ...
        if (($options->summary_rating || $options->view_mode == 'slider_lite') && count($social_biz) > 0) {
            ...
            array_unshift($businesses, $this->merge_biz($social_biz, 'summary', $summary_name, $summary_url, $summary_photo, $summary_wr, 'summary'));
        }
    ...
    public function get_google_reviews_all($google_biz, $options) {
        ...
        $google_api_url .= '&limit=' . (strlen($options->google_api_limit) > 0 ? $options->google_api_limit : '100');

        $google_response_json = $this->helper->json_remote_get($google_api_url);
    ...
    public function get_facebook_reviews($facebook_biz, $page_access_token, $options) {
        $business = null;
        $reviews = array();

        $fb_api_limit = strlen($options->fb_api_limit) > 0 ? $options->fb_api_limit : '100';

        $facebook_api_url = BRB_FACEBOOK_API . $facebook_biz->id . "?access_token=" . $page_access_token . "&fields=ratings.fields(reviewer{id,name,picture.width(" . $options->reviewer_avatar_size . ").height(" . $options->reviewer_avatar_size . ")},created_time,rating,recommendation_type,review_text,open_graph_story{id}).limit(" . $fb_api_limit . "),overall_star_rating";

        if (!$options->fb_rating_calc) {
            $facebook_api_url = $facebook_api_url . ",rating_count";
        }

        $facebook_response_json = $this->helper->json_remote_get($facebook_api_url);
